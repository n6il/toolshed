...
extern direct unsigned HadArrow, IsExtrn;
extern direct char **S_Addrs, *CodeSize, Pass2, d005e, d005f, *SrcChar;
extern direct short GlobCnt;
extern direct char LblTyp;
extern direct struct symblstr **d0078, *SmblDsc;

extern char    *AltLbl;
extern struct symblstr d0587;

extern struct symblstr *d078f, **d07b1;
...
#endif
{
 register struct ref_str *new;
   ...
   myref->wrd = new;
      ...
      /* straight asm, how do we do otherwise ???
       * EASY!!!!!!!!!!!!!*/
#ifdef __STDC__
...
#endif
{
 ...
 struct symblstr *var;
   ...
   if ( d078f ) {
      nwtree = d078f;
      d078f = d078f->left;
      /*nwtree->left = nwtree->wrd = nwtree->smbltyp = nwtree->w1 = nwtree->s_ofst = 0;*/
      memset(nwtree,0,sizeof(struct symblstr));  /* easier */
   }
   else {
      nwtree = (struct symblstr *)getmem(sizeof(struct symblstr));
      /*nwtree->left = nwtree->right = nwtree->wrd = nwtree->smbltyp = nwtree->w1 = nwtree->s_ofst = 0;*/
      memset(nwtree,0,sizeof(struct symblstr));
   }
   if ( var = *oldtree ) {    /* if an oldtree exists */
      if ( cmp_cas < 0 ) {
         nwtree->right = var;
         var->left = nwtree;
      }
...
/* FndTreEmty() -Trace Tree down to find an empty space at the right
    place and return strcmp() result, or, return 0 if this name
...
/* The following doesn't seem exactly right, but it works (for OSK)
   what we're taking is the contents of d0078 and adding to it sum, etc
   - d0078 seems to be a pointer to an array of struct smblstructs */
