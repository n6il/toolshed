...
/*extern direct unsigned NumBytes, jsrOfst, d0021;
extern direct long nmbr_int;
extern direct short d0033;
extern direct int HadBrkt, HadArrow, d0039;
extern direct unsigned CodeSize, undeflbl;
...
extern direct char     d0063, *SrcChar, *d007c;
...
/* getreg(PhsPul) = l102b() : Parses the Register name structure to match up 
        ...
        returns: if not PshPul
                 ...
                 if PshPul
                   push/pull code for register (0 if invalid)
*/
 ...
 char var2;
   ...
   while ( regptr < REGCOUNT ) {
      if ( RegCode[regptr].regnam[0] == _toupper( *SrcChar ) ) {
         ...
         if ( PshPul == 0 ) {
            if ( regptr < 8 ) {  /* Not register (X,Y,U,S,PC) */
               return ( regptr | 8 );
            } else {
   ...
   if ( (var2 = regopt->RAddr)->w1 & 2 ) {
      ...
      while ( ++var1 < OptBPtr ) {
         if ( (var2 = var1->RAddr)->w1 & 2 ) {
               ...
               /* NOTE: the following cast for "var3" is a temporary
                        kludge... it works, but see if can do better */
