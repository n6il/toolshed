...
extern direct char *Bsr_Ptr, *Cod_End;
extern CODFORM S_Lbr, S_Nam, S_Bsr;
extern JMPTBL Drectives[];*/
/*extern char *O_Nam, d07c9[], *SrcNam[];
extern char **LastSrc;*/
...
MACDSCR *var1;
   ...
   d005f = d005e;  LblTyp = d0060;  d0063 = d0062;
   OptBPtr = &d0791[0];
   ...
   if (MovLbl(LabelName) != 0 ) {
      ...
      if ( (d00d4==0) && ((var1=McNamCmp(LabelName))!=0) ) {
         ...
         if ( (Cod_Ent=FindCmd( LabelName, S_Nam, SNamEnd )) ) {
            ...
            if ( d00d4 != 0 ) {
            ...
            else {                          /* in case of macro stuff */
               if ( InMacro != FALSE ) {
                  if ( jsrOfst == 4 ) {
                     return( nstmacdf() );   /* can't nest macro defs */
                  }
                  if ( jsrOfst == 5 ) {                  /* if "endm" */
                     add_nul();
                     InMacro = 0;
                     return 1;
                  }
               ...
               if ( _IfIsTru !=0 ) {
                  if ( jsrOfst != 1 ) { /* if not "if" statememt */
...
#endif
{
 register char *regvar;
   ...
   if ( Label || operatr || (NumBytes>0) ) {      /* l05e6 */
    int var1;
      regvar = CmdCod;
	 ...
	 printf("%02x",MLCod.opcode&0xff);++var1;
	 regvar = MLCod.opers;
	 for(; var1 < 5; var1++) {
		 if(var1<NumBytes)
			 printf( "%02x", (*(regvar++))&0xff);
