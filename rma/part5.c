...
/*extern direct int  PgDepth, PgWidth, NumBytes;
extern direct char *Lbr_Ptr, *Rmb3_Ptr;
extern direct FILE *SrcFile;
extern direct unsigned d0027;
extern direct long nmbr_int;
extern direct int  doList, dGlbls, edatas, ddatas;
extern direct char **Glbls, **S_Addrs, **Adrs_Ptr;
extern direct char Pass2, d005e, d005f, *SrcChar;
extern direct int  d0051, *d0053;
extern direct short comn_cnt;
extern direct char d0060, d0062, *Label, *operatr, *oprand, *coment;
extern direct int ((**jmp_ptr)())[];
extern direct char *CodTbBgn, *CodTbEnd, *d007c;
extern direct struct symblstr d0092, d00a6;
extern direct char ListFlg, C_Flg, OFlg, ff_flg, g_flg,
                   e_flg, S_Flg, x_flg, R_Keep;
extern direct char listON;
extern direct int  CmdCod;

extern direct char MLCod.opcode;
extern direct union storer1 *opr_ptr;
extern direct int MacFlag, d00d2, d00d4, macfp2;

extern direct struct symblstr *SmblDsc;

extern CODFORM S_Psec, Data_Equ, srmb3;
extern JMPTBL  drectives[], j_secs, d035c[];
extern FILE **ThisFile;
extern char _ttl[];
extern struct symblstr d0587;
...
/*   d0387[3], d038a[3], d038d[3], d0390[3],
     d0393[3], d0396[3], d0399[3];*/
...
#endif
{
   if ( l1723() ) {
      *(Adrs_Ptr = &d0051) = l174f();
      ...
      l175f( d0051+nmbr_int );  /* check out to see if long needed */
      return 1;
   }
   return 0;
}
...
#endif
{
 ...
 int flag = 0;          /* CHECK THIS OUT .. I THINK it's right */
...
/*   CmdCod = &(nmbr_int) + 1;
   CmdCod = &nmbr_int;
   CmdCod +=3;*/ /* Why not 1??? */ /* because nmbr_int is Long */
	CmdCod = nmbr_str;
   l18cd(l2387,1);
}
...
{
   /*CmdCod = &nmbr_int;
   CmdCod +=2;*/ /* Why???  because nmbr_int is Long */
	CmdCod = nmbr_str;
   l18cd(l2369,2);
}
 ...
 register struct optns *tcmd;
   ...
   for ( ;; ) {
      ...
      while ( tcmd < endof(cmdopts) ) {
         if ( t_char == tcmd->optn ) {
            *tcmd->optadr += ch_opt;
            ++SrcChar;   /*  Bump for nxt chr (fixed from orig)*/
            break;
         }
         ++tcmd;
      }
      if ( tcmd >= endof(cmdopts) ) {
       ...
       /* Original pre-bump here moved up to while... loop above
          to make d-w-v opts work correctly  */
      if ( *SrcChar != ',' ) {
         break;
      }
      t_char = *(++SrcChar);
