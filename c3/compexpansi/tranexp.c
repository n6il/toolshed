/*
 * tranexp.c - Translate Expressions
 *
 * C3 C Compiler
 */
...
static void stackx(expnode *node);
static void assop(expnode *node, int op);
static void doass(expnode *node);
static void tranbinop(int op, expnode *node);
static void loadxexp(expnode *p);
static void dobool(expnode *tree);
static void tranxexp(expnode *node);
static void doload(expnode *tree);
...
static void dobool(expnode *tree)
{
	...
	gen(JMP, l1.labnum=getlabel(), 1, NULL);
...
void doquery(expnode *tree, void (*loadfunc)(struct expstr *))
{
	...
	gen(JMP, l1.labnum=getlabel(), 0, NULL);
	...
	for (rhs = node; rhs = rhs->right;) {
		...
		if (islong(lhs)) {
			if (lhs->op == LCONST) pushcon(lhs);
				...
				lload(lhs);
				gen(LONGOP, STACK, 0, NULL);
			}
		}
		else if (isfloat(lhs)) {
			dload(lhs);
			gen(DBLOP, STACK, 0, NULL);
		}
		else {
...
ord:
		...
		if (lhs->type == CHAR) gen(CTOI, 0, 0, NULL);
		switch (op) {
			case AND:
			case OR:
			case XOR:
