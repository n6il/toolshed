	...
	char           *argv[];
{
	...
	theClock = time((long *) 0);
	timdat = ctime(&theClock);	/* produce time/date string */
	...
	for (i = 1; i < argc; ++i)	/* Loop for each arg */
	{
		if (*argv[i] == '-')	/* If arg is a switch... */
		{
			...
			case 'E':	/* Format: -e[=<decimal number>] */
				...
				break;
			case 'l':	/* Copy source to ASM output
					 * (Microware only) */
			case 'L':
				lflag = TRUE;	/* select Microware C
						 * compatible */
				break;
			case 'v':
				incdirs[incdircount++] = argv[i] + 3;
				lflag = TRUE;
				break;
	...
	else if ((fpath[fptr] = open(argv[namptr], O_RDONLY)) != ERROR)
		strcpy(ifnbuf[fptr], argv[namptr]);	/* store root file name */
	else
		/*
		 * exit(_errmsg(errno,"Cannot open source file:
		 * %s\n",argv[namptr]));
		 */
	{
	...
	lnprint(3, edition[0], NULL);	/* print root file #line info */
	for (;;)		/* Loop to process each file in IFNBUF[] */
	{
		...
		for (;;)	/* Loop to process each line in current file */
		{
			if (eflag)	/* break if EOF */
				break;
			if (!(a = getln(0)))	/* Fill LINE[] up to CR or
						 * EOF */
			{
				lnflag[1] = -1;	/* cancel #line after current
						 * line */
				lnflag[0] = _line_;	/* print #line before
							 * next line */
				continue;	/* empty line */
			}
			if (lflag)	/* generate Microware codes to copy
					 * source lines */
			{
				if (cflag)
			...
			a = wspace(a);
			if (*line == '#' || line[1] == '#')
				a = prep();	/* Parse preprocessor
						 * commands */
			if (a && procsw && tstline(a))
			{
				splittok(line, 0);	/* Split non # line into
							 * tokens */
				expand(line, NULL, NULL);
				cncatstr(line);	/* Concatenate adjacent
						 * strings */
				convert(VTAB, SPACE);
				lnprint(0, lnflag[0], NULL);
				printf("%s\n", line + 1);
				lnprint(0, lnflag[1], NULL);
			}
			else
			{
				lnflag[1] = -1;	/* cancel #line after current
		...
		lnprint(2, 0, NULL);	/* print #line code to reallign to
					 * old file */
	}
...
/*
 * Get next line from input path up to CR or EOF.  If EOF then eflag is TRUE.
 * 'a' is the position in line[] to start reading in new input. Upon exit 'a'
 * = position of null terminator in line[].  All lines end with a NULL, even
 * if EOF occurs line is  NULL terminated.  CRs are stripped. The only
 * exception is when an EOF occurs when a=0 then since no chars are waiting
 * to be processed getln() exits.  This is only rtn that sets eflag. If line
 * exceeds line buffer length then fatal error occurs.
 */

int             getln(a)	/* gets next line from input path, strips CR,
				 * and NULL */
	int             a;	/* terminates */
{
	register char  *lnptr;
	...
	int             a;
{
	...
	c = 1;
	while ((b = findstr(c, line, "??")) != 0)
	{
		if ((d = gettri(b)) != 0)	/* Get replacement char or
						 * zero if invalid */
		{
...
int             gettri(b)	/* Given b=pos of first ? (1-...) in trigraph */
...
int             xtndln(a)	/* concatenate lines ending in \ */
	register int    a;
{
	if (line[a - 1] == '\\')
	{
		...
		else
		{
			...
			lnflag[1] = _line_;	/* set to print #line after
						 * line */
		}
	}
	return a;
...
int             wspace(a)	/* Shrink whitespace chars to single space */
	int             a;
{
	register char  *b;
	char           *c;
...
void             convert(x, y)	/* convert all occurrences of x to y */
	char            x,
	                y;	/* Routine will fail to operate properly if
				 * x=y */
{
	register char  *b;
...
int             cmnt(a)		/* strip comments from line */
	int             a;
{
	int             b;
	...
	while (line[b])		/* read until NULL terminator */
	{
		if (line[b] == '/')
			...
			a = space(a, b);	/* Reduce multiple spaces to
						 * one */
		++b;
	}
	return a;
	               ...
	               *b;
{
	...
	if (line[*b + 1] == '*')/* found start of comment */
	{
		...
		if ((c = findstr(*b + 3, line, "*/")) != 0)	/* Cmnt ends on same
								 * line */
		{
			strcpy(&line[*b + 1], &line[*b + 3 + c + 1]);
			a -= (c - *b);
			return a;
		}
...
int             qa(ln, x, b)	/* Searches ln for end " or ' depending on x */
	char           *ln,
	                x;	/* b = first " or ' */
	int             b;
{
...
int             qa2(ln, x, b)	/* Searches ln for end " or ' depending on x */
	char           *ln,
	                x;	/* b = first " or '  Swaps VTAB for spaces */
	int             b;
{
	register char  *lnptr;
	...
	if (*lnptr == '\\')
	{
		lnptr = escseq(ln, lnptr);	/* bypass any escape
						 * sequences */
		b = qa(ln, x, lnptr - ln);	/* skip to char after \ */
	}
	else if (*lnptr != x)
...
int             space(a, b)	/* shrink spaces to one */
	int             a,
	                b;
{
	register char  *cptr;
...
int             usage(void)
{
	fputs("Usage:   c3prep [-opt] <source file>\n\n", stderr);
	...
	fputs("         -v=<include-path> additional paths to search for includes\n", stderr);
	fputs("\nc3prep operates in Microware compatible mode unless\n", stderr);
	fputs("the -c switch is invoked.\n", stderr);
...
int             getident(ln, b)	/* Given B: pos. of first char. in identifier */
	char           *ln;
	register int    b;	/* Returns B: ERROR = invalid identifier or
				 * ## = pos of end */
...
#ifndef M6809
int             skpbl(ln, a)	/* Given B: start pos. of blank search */
	char           *ln;
	int             a;	/* Returns B: pos. of first non-blank char.
				 * after spaces */
{
	while (ln[a] == ' ')
		a++;
...
#1
	...
	int             rskpbl(ln, a)	/* Given A: end pos. to start search */
	char           *ln;
	register int    a;	/* Returns A: pos. of first non-blank char.
				 * after spaces */
{
	while (ln[a] == ' ')
		--a;
...
char           *escseq(l, ln)	/* returns char * to last char in escape
				 * sequence */
	char           *l,
	               *ln;	/* ln must point to first \ in escape
				 * sequence */
{
	switch (*++ln)
	{
