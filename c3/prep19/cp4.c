...
void expand(ln, loop, lpcntr)	/* Macro expansion rtn */
	char           *ln;	/* ln[] must start and end with a space
				 * character */
	int            *loop,
	               *lpcntr;
{
	 ...
	 * resulted in one of the most dificult, time consuming bugs I've
	 * ever traced. It was ok if the caller had a "0" in the third
	 * argument position, but crashed if the caller used a "NULL". The
	 * problem then was that the data at address $0000 was not zero at
	 * times which resulted if the vars loop and lpcntr being left at
	 * $0000. The input file pathtable lived near $0000, and got trashed
	 * because the writes this routine did were then at or near $0000.
	 * Gene Heskett, WDTV5CE.
	 */
	if (!lpcntr)
	{
		lpc = 0;
	...
	for (x = 0; x < idcntr; ++x)	/* loop for each identifier */
	{
		...
		for (i = 0; i < defcntr; ++i)	/* loop for each definition
						 * name */
		{
			/*
			...
			*/
			if (strcmp2(idptr[x], defnam[i]))	/* returns 1 if match
								 * else 0 */
			{
				lpflag = FALSE;
				for (n = 0; n < *lpcntr; ++n)
				...
				*buf = 0;
				if (defarg[i])	/* If a macro */
				{
					...
					for (j = 0; j < e; ++j)	/* loop to expand each
								 * arg */
					{
						/*
					...
					*/
					...
					strcpy(lnptr, buf);	/* put expanded tok-seq
								 * in ln[] */
				...
				{
					/* Not a macro */
					mname = strlen(defnam[i]) - 1;	/* get length of
									 * identifier */
					...
					case 1:	/* __FILE__ */
						...
						break;
					default:
						strcpy(lptr, deftok[i]);	/* copy tok-seq into
										 * line */
...
void expln(ln, l, lc)		/* Inserts beginning and ending space then
				 * calls expand */
	char           *ln;
	int            *l,
	               *lc;
