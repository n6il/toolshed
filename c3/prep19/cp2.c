...
int gettoken(ln, b)			/* b=pos (0-...) to search ln; assumes b
				 * points to 1st */
	char           *ln;	/* char of token (no error chking except EOL) */
	int             b;	/* ln=search $tring */
{				/* Returns last char. pos of token (offset
				 * from ln) */
	...
	if ((i = findchar(lnptr, ' ')) != 0)	/* token terminated w/space */
		return i - 2 + b;
...
int toksrch(ln, tk, pv, kv, bv, b, tcnt)	/* Searches for specified token under */
	char           *ln,
	               *tk;	/* specified conditions.  */
	int             pv,
	                kv,
	                bv,
	                b,
	               *tcnt;	/* ln = search $tring  tk = token to search
				 * for */
{				/* pv,kv,bv = (),[],{} cntrs (++/-- for
				 * open/close */
	int             c,
	                d,
	                mflag;	/* b = start pos of srch (0-...) must be 1st
				 * char */
	int             pcnt,
	                kcnt,
	                bcnt;	/* of token (no error chking) */
	register char  *lnptr;	/* pv,kv,bv must match pcnt,kcnt,bcnt for
				 * exit */

	/* also tk must match current token for exit */
	*tcnt = 0;		/* If pv,kv or bv = 99 then that flag can
				 * have any */
	pcnt = kcnt = bcnt = 0;	/* value upon exit (i.e. wildcard)  */
	do			/* Returns pos of first char in token or -1
				 * (fails) */
	{
		mflag = 0;	/* no match */
		...
		if (pv == 99)	/* if any of these are 99 they automatically
				 * match */
			pcnt = 99;
		if (kv == 99)
			kcnt = 99;
...
int tstargs(i)			/* count # of args for defargs[i] */
	int             i;
{
	char           *ptr;
	int             b;
...
void addqmac(mac)			/* Adds " to macro when # is found */
	char           *mac;
{
	char            buf[MAX_LENGTH],
	               *bptr;
...
char           *getoknum(tok, buf, num)	/* Counts tokens in tok[] for token #
					 * num */
	char           *tok,
	               *buf;	/* and stores it in buf[].  Returns NULL  */
...
void putdtbl(b, c)			/* Given B: Start pos. of identifier/ C: end
				 * pos. */
	int             b,
	                c;	/* Puts identifier in define $trng tbl */
{
	register int    i;
...
void dodefine(a, b)			/* Given A:strlen, B: pos. of char. after 1st
				 * space */
	int             a,
	                b;	/* This rtn places define parms into proper
				 * tables */
{				/* for reading during macro expansion */
	int             c,
	                d;	/* Returns NULL if no errors */

	if ((d = c = getident(line, b)) == ERROR)	/* Get identifier */
	{
		doerr(1, b);	/* Return if invalid identifier */
	...
	{
		d = b = skpbl(line, c + 2);	/* Align to 1st non-space
						 * char in macro */
		if (line[b] != ')')	/* if more than 0 arguments in macro */
		{
			...
			do	/* Scan argument format */
			{
				...
				{
					doerr(2, b);	/* return if any args missing */
	...
	{
		line[b + 1] = ' ';
		line[b + 2] = '1';	/* This does not test LINE to see if
					 * it is full */
		line[b + 3] = '\n';
	}
...
char prep(void)				/* Check for preprocessor commands */
{
	int             c;
	char           *ln;

	ln = line;
	while (*(++ln) == '#' || *ln == ' ')	/* locate first directive
						 * character */
		;
	...
	if (procsw)
	{
		if (strcmp2(ln, "define "))
		{
			...
			/*
			 * fprintf(stderr,"PREP (after dodef):
			 * line=|%s|\n",line);
			 */
		...
		if (strcmp2(ln, "undef "))
		{
			...
			doundef(&ln[6]);	/* remove undef identifier
						 * from def table */
		...
		if (strcmp2(ln, "asm"))
		{
			for (;;)/* send all following lines through for
				 * assembler */
			{
				getln(0);
				if (eflag)
					break;
				if (findstr(1, line, "#endasm") != 0)
					break;
				if (cflag)
					puts("#2");
				else
			...
			if (eflag && cflag)	/* error only in Microware
						 * mode (no #endasm) */
				doerr(18, 1);
			return (killine());
		}
