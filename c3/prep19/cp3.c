	...
	char           *ln;
{
	...
	if (procsw)
		ifstat[++iflvl] = procsw = gettorf(ln);	/* evaluate IF & return
							 * T or F */
	else
	...
	char           *ln;
{
	...
	if (ifstat[iflvl] == FALSE)
		procsw = ifstat[iflvl] = gettorf(ln);	/* evaluate IF & return
							 * T or F */
	ifnest[iflvl] = 2;
...
int ifcalc(ln)			/* processes #if/#elif expression and returns
				 * TRUE or FALSE */
	char           *ln;
{
	unsigned        x;
...
void dodfined(ln)			/* replaces defined commands with 0 or 1 */
	char           *ln;
{
	int             b,
	                c,
	...
	pflag = FALSE;
	while ((b = findstr(1, ln, " defined ")) != 0)
	{
		...
		d = dodef(ln, c - 1);	/* returns TRUE if defined, FALSE if
					 * not defined */
	                ...
	                errptr;
{
	...
	case 7:
		...
		/*
		 * UNUSED ERRORS case 8: fprintf(stderr,"Compiler error -
		 * line too long"); xflag=TRUE; break;
		 */
	case 9:
	...
	case 11:
		...
		/*
		 * UNUSED case 12: exit(fprintf(stderr,"Syntax error -
		 * unmatched () or []")); break;
		 */
	case 13:
...
#of bytes read value)
			                    ...
			                    tfr y, d
			                    leax d, x(add
#of bytes read to &s and get end of string)
						                      lda, x
						                      cmpa
#13    (check for CR)
